...
export async function writeUint8ArrayToTempFile(data: Uint8Array, fileName: string) {
  await fs.writeFile(getTempFilePath(fileName), data);
}

export async function readUint8ArrayFromTempFile(fileName: string): Promise<Uint8Array> {
